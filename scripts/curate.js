...
async function main() {
  ...
  const itemList = rawItems.map((item, i) => {
    ...
    // 摘要给到 400 字：价格/上市时间这类关键信息经常出现在 150 字之后，
    // 实测放宽后含价格线索的候选从 8% 升到 15%
...
**宁缺勿滥**：如果某一层在候选池里确实找不到合格内容，就少返回一条，
不要用不相关的内容凑数，更不要把纯软件/开发工具硬说成硬件产品。
少一条比错一条好，页面允许某一层只有 1 条甚至空着。
...
- 第 4-5 条【AI 硬件】：AI相关的消费级实体硬件——AI可穿戴、AI陪伴设备、AI相机、具身智能/机器人、AI交互终端。
  **必须有可购买或可预期上市的实体硬件载体。**以下一律不算 AI 硬件，绝对不要选：
  模型/论文/benchmark/prompt 技巧、GitHub 仓库、开发框架、AI 编程工具、
  跑在通用电脑或手机上的 AI 软件、云服务、AI 芯片本身（除非是消费终端产品）。
  这一层没有合格内容时就少返回，不要用软件内容填充。
...
- "index": 原始编号（整数）
- "brief": 一段式概要（40-70字，把产品名、卖点、为什么值得关注融合成一段话。用 **加粗** 标记其中最关键的产品名或核心卖点，最多加粗2处。

  **关键信息优先**：如果候选摘要里明确写了下列信息，务必写进概要——读者可能打不开原文链接，
  只能靠这段概要判断值不值得深挖：
  价格 / 涨降价幅度、上市或预售时间、众筹金额与达成率、交付时间、具体规格数字（重量、容量、功率）。

  **严格约束**：只能写候选摘要里原文出现的数字，一个字都不许推测、换算或补全。
  摘要里没写价格就不要提价格，没写时间就不要提时间——绝大多数候选是没有这些信息的，
  那就正常写卖点，这是完全可以接受的。编造一个价格比不写价格糟糕得多。）
