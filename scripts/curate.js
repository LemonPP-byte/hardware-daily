...
async function main() {
  ...
  const prompt = `你是一个消费硬件产品经理的每日选品助手。从以下今日抓取的内容中，选出最值得关注的 8 条，并为每条评分。

你的目标用户是：消费电子/硬件产品经理，关注「什么新产品能打动消费者」「什么新形态能激发购买欲」。

8 条内容的结构（严格按此分层，按顺序排列）：
- 第 1-2 条【成熟品牌】：已有市场验证的品牌的新品动态（如 Apple、Sony、Dyson、Nothing、Bose、Framework、Teenage Engineering、Razer、Anker 等有出货量的品牌）。优先选有新意的产品线延伸、联名合作、新品类尝试。
- 第 3-4 条【AI 硬件】：和 AI 相关的消费级硬件产品——AI 耳机、AI 眼镜、AI 玩具、AI 宠物设备、AI 相机、AI 穿戴、AI 家居设备等。必须是实体硬件产品，不是纯软件/模型/API。
- 第 5-6 条【新锐产品】：已经有实际交付能力或众筹成绩亮眼（$50k+）的新品牌/新产品。必须已经是可购买或即将可购买的状态，不要纯概念。
- 第 7-8 条【野生灵感】：可以更发散，小众设计品、意想不到的硬件创新、有趣的品牌跨界。必须有消费者实际讨论。
...
只返回 JSON 数组，不要其他文字。`;
  ...
  const tierByPosition = ['成熟品牌','成熟品牌','AI 硬件','AI 硬件','新锐产品','新锐产品','野生灵感','野生灵感'];
