...
async function main() {
  ...
  const prompt = `你是一个消费硬件产品经理的每日选品助手。从以下今日抓取的内容中，选出最值得关注的 6 条，并为每条评分。

你的目标用户是：消费电子/硬件产品经理，关注「什么新产品能打动消费者」「什么新形态能激发购买欲」。

6 条内容的结构（严格按此分层，按顺序排列）：
...
- 第 3-4 条【新锐产品】：已经有实际交付能力或众筹成绩亮眼（$50k+）的新品牌/新产品。必须已经是可购买或即将可购买的状态，不要纯概念。
- 第 5-6 条【野生灵感】：可以更发散，小众设计品、意想不到的硬件创新、有趣的品牌跨界。必须有消费者实际讨论。
...
请返回严格的 JSON 数组，包含 6 个对象（按上述 1-2-3-4-5-6 结构排列），每个对象字段：
- "index": 原始编号（整数）
